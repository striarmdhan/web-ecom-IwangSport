                        ...
                        <tbody>
                            @foreach ($orders as $order)
                                @php
                                    ...
                                    if ($order->status == 'diporses') {
                                        $status =
                                            '<span class="bg-yellow-500 py-1 px-3 rounded text-white shadow">DiPoroses</span>';
                                    }
                                    ...
                                    if ($order->status == 'terkirim') {
                                        $status =
                                            '<span class="bg-blue-700 py-1 px-3 rounded text-white shadow">Terkirim</span>';
                                    }
                                    if ($order->status == 'dibatalkan') {
                                        $status =
                                            '<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Dibataklan</span>';
                                    }
                                    ...
                                    if ($order->payment_status == 'paid') {
                                        $payment_status =
                                            '<span class="bg-green-600 py-1 px-3 rounded text-white shadow">Lunas</span>';
                                    }
                                    if ($order->payment_status == 'faile') {
                                        $payment_status =
                                            '<span class="bg-red-600 py-1 px-3 rounded text-white shadow">Gagal</span>';
                                    }
                                @endphp
